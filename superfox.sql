...
-- Schema superfox
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `superfox` DEFAULT CHARACTER SET latin1 ;
USE `superfox` ;
...
CREATE TABLE IF NOT EXISTS `superfox`.`funcionario` (
  ...
  PRIMARY KEY (`pk_funcionario`, `fk_cargo`),
...
CREATE TABLE IF NOT EXISTS `superfox`.`usuario` (
  ...
  INDEX `fk_usuario_funcionario_idx` (`fk_funcionario` ASC),
...
DELIMITER $$
USE `superfox`$$
CREATE DEFINER = CURRENT_USER TRIGGER `superfox`.`itens_compra_AFTER_INSERT` AFTER INSERT ON `itens_compra` FOR EACH ROW
BEGIN
...
USE `superfox`$$
CREATE DEFINER = CURRENT_USER TRIGGER `superfox`.`itens_compra_AFTER_DELETE` AFTER DELETE ON `itens_compra` FOR EACH ROW
BEGIN
